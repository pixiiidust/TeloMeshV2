...
def build_graph(input_csv="outputs/session_flows.csv", output_graph="outputs/user_graph.gpickle"):
    """
    ...
    Returns:
        nx.DiGraph: The directed graph representing user journeys.
    """
...
def main():
    ...
    G = build_graph(args.input, args.output)
...
if __name__ == "__main__":
    ...
    G = build_graph(args.input_path, args.output_path)
    
    # Print graph statistics
    num_nodes = len(G.nodes)
    num_edges = len(G.edges)
    
    print(f"✅ Graph built with {num_nodes} nodes and {num_edges} edges → {args.output_path}")
